$(document).ready(function () {
    ...
    function ReloadGrid() {
        $("#list").setGridParam({ url: base_url + 'CompanyInfo/GetList', postData: { filters: null }, page: 'first' }).trigger("reloadGrid");
    }
    ...
    //GRID +++++++++++++++
    $("#list").jqGrid({
        url: base_url + 'CompanyInfo/GetList',
        datatype: "json",
        colNames: ['ID', 'Name', 'Address', 'City', 'Postal Code', 'Phone No.', 'Fax No.', 'Email', 'Website', 'NPWP', 'NPWP Date', 'Created At', 'Updated At'],
        colModel: [
                  ...
                  { name: 'website', index: 'website', width: 180 },
                  { name: 'npwp', index: 'npwp', width: 180 },
                  { name: 'npwpdate', index: 'npwpdate', search: false, width: 100, align: "center", formatter: 'date', formatoptions: { srcformat: 'Y-m-d', newformat: 'm/d/Y' } },
    ...
    $('#btn_add_new').click(function () {
        ...
        $('#NPWPDate').datebox('setValue', $.datepicker.formatDate('mm/dd/yy', new Date()));
    ...
    $('#btn_edit').click(function () {
        ...
        if (id) {
            ...
            $.ajax({
                dataType: "json",
                url: base_url + "CompanyInfo/GetInfo?Id=" + id,
    ...
    $('#delete_confirm_btn_submit').click(function () {
        ...
        $.ajax({
            url: base_url + "CompanyInfo/Delete",
            type: "POST",
    ...
    $("#form_btn_save").click(function () {
        ...
        if (id != undefined && id != '' && !isNaN(id) && id > 0) {
            submitURL = base_url + 'CompanyInfo/Update';
        }
            // Insert
        else {
            submitURL = base_url + 'CompanyInfo/Insert';
        }
        ...
        $.ajax({
            ...
            data: JSON.stringify({
                ...
                Email: $("#Email").val(), Website: $("#Website").val(), NPWP: $("#NPWP").val(), NPWPDate: $('#NPWPDate').datebox('getValue'),
            }),
            async: false,
            cache: false,
            timeout: 30000,
