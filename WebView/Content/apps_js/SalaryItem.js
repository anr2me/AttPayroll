$(document).ready(function () {
    ...
    function ReloadGrid() {
        ...
        $("#list").setGridParam({ url: base_url + 'SalaryItem/GetList', postData: { filters: null, ParentId: value }, page: '1' }).trigger("reloadGrid");
    }
    ...
    //    $.ajax({
    //        dataType: "json",
    //        url: base_url + 'SalaryItem/GetListDepartment?Id=' + branchID,
    //        async: false,
    ...
    $("#list").jqGrid({
        ...
        datatype: "json",
        colNames: ['ID', 'Code', 'Name', 'Description', 'Is Legacy', 'Type', 'Status', 'Created At', 'Updated At'],
        colModel: [
    			  { name: 'id', index: 'id', width: 80, align: "center" },
                  { name: 'code', index: 'code', width: 100 },
				  { name: 'name', index: 'name', width: 180 },
                  { name: 'description', index: 'description', width: 250 },
                  { name: 'islegacy', index: 'islegacy', width: 100, stype: 'select', editoptions: { value: ":;false:No;true:Yes" } },
                  { name: 'salaryitemtype', index: 'salaryitemtype', width: 150, stype: 'select', editoptions: { value: getSelectOption("#SalaryItemType") } },
                  { name: 'salaryitemstatus', index: 'salaryitemstatus', width: 100, stype: 'select', editoptions: { value: getSelectOption("#SalaryItemStatus") } },
        ...
        gridComplete:
		  function () {
		      ...
		      for (var i = 0; i < ids.length; i++) {
		          ...
		          rowItemType = document.getElementById("SalaryItemType").options[$(this).getRowData(cl).salaryitemtype].text;
		          $(this).jqGrid('setRowData', ids[i], { salaryitemtype: rowItemType });

		          rowItemStatus = document.getElementById("SalaryItemStatus").options[$(this).getRowData(cl).salaryitemstatus].text;
		          $(this).jqGrid('setRowData', ids[i], { salaryitemstatus: rowItemStatus });
		      }
		      //var ids = $(this).jqGrid('getDataIDs');
		      //for (var i = 0; i < ids.length; i++) {
		      //    var cl = ids[i];
    ...
    $('#btn_edit').click(function () {
        ...
        if (id) {
            ...
            $.ajax({
                dataType: "json",
                url: base_url + "SalaryItem/GetInfo?Id=" + id,
                success: function (result) {
                    if (result.Id == null) {
    ...
    $('#delete_confirm_btn_submit').click(function () {
        ...
        $.ajax({
            url: base_url + "SalaryItem/Delete",
            type: "POST",
    ...
    $("#form_btn_save").click(function () {
        ...
        if (id != undefined && id != '' && !isNaN(id) && id > 0) {
            submitURL = base_url + 'SalaryItem/Update';
        }
            // Insert
        else {
            submitURL = base_url + 'SalaryItem/Insert';
        }
