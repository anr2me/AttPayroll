$(document).ready(function () {
    ...
    function ReloadGrid() {
        ...
        $("#list").setGridParam({ url: base_url + 'OtherExpense/GetList', postData: { filters: null, ParentId: value }, page: 'first' }).trigger("reloadGrid");
    }

    function ReloadGridDetail() {
        $("#listdetail").setGridParam({ url: base_url + 'OtherExpense/GetListDetail?Id=' + $("#id").val(), postData: { filters: null } }).trigger("reloadGrid");
    }
    ...
    //GRID +++++++++++++++
    $("#list").jqGrid({
        url: base_url + 'OtherExpense/GetList',
    ...
    $('#btn_edit').click(function () {
        ...
        if (id) {
            ...
            $.ajax({
                dataType: "json",
                url: base_url + "OtherExpense/GetInfo?Id=" + id,
    ...
    $('#delete_confirm_btn_submit').click(function () {
        $.ajax({
            url: base_url + "OtherExpense/Delete",
            type: "POST",
    ...
    $("#form_btn_save").click(function () {
        ...
        if (id != undefined && id != '' && !isNaN(id) && id > 0) {
            submitURL = base_url + 'OtherExpense/Update';
        }
            // Insert
        else {
            submitURL = base_url + 'OtherExpense/Insert';
        }
    ...
    $("#listdetail").jqGrid({
        url: base_url,
        datatype: "json",
        colNames: ['OtherExpense ID', 'Employee ID', 'Employee NIK', 'Employee Name', 'Amount', 'Effective Date', 'Recurring', 'Remark'],
        colModel: [
				  { name: 'otherexpenseid', index: 'otherexpenseid', hidden: true, width: 80, sortable: false },
                ...
                success: function (result) {
                    if (result.Id == null) {
                        ...
                        if (JSON.stringify(result.Errors) != '{}') {
                            ...
                            $('#id').val(result.Id);
                            $('#OtherExpenseId').val(result.OtherExpenseId);
    ...
    $('#btn_edit_detail').click(function () {
        ...
        if (id) {
            $.ajax({
                dataType: "json",
                url: base_url + "OtherExpense/GetInfoDetail?Id=" + id,
                success: function (result) {
                    if (result.Id == null) {
                        $.messager.alert('Information', 'Data Not Found...!!', 'info');
                    }
    ...
    $('#btn_del_detail').click(function () {
        ...
        if (id) {
            ...
            $.messager.confirm('Confirm', 'Are you sure you want to delete record?', function (r) {
                if (r) {
                    $.ajax({
                        url: base_url + "OtherExpense/DeleteDetail",
                        type: "POST",
                        contentType: "application/json",
                        data: JSON.stringify({
                            Id: id,
    ...
    $("#detail_btn_submit").click(function () {
        ...
        // Update
        if (id != undefined && id != '' && !isNaN(id) && id > 0) {
            submitURL = base_url + 'OtherExpense/UpdateDetail';
        }
            // Insert
        else {
            submitURL = base_url + 'OtherExpense/InsertDetail';
        }
        ...
        $.ajax({
            ...
            url: submitURL,
            data: JSON.stringify({
                Id: id, OtherExpenseId: $("#id").val(), EmployeeId: $("#EmployeeId").val(), Amount: $("#Amount").numberbox('getValue'),
