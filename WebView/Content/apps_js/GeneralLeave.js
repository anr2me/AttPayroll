$(document).ready(function () {
    ...
    function ReloadGrid() {
        ...
        $("#list").setGridParam({ url: base_url + 'GeneralLeave/GetList', postData: { filters: null, ParentId: value }, page: 'first' }).trigger("reloadGrid");
    }

    function ReloadGridDetail() {
        $("#listdetail").setGridParam({ url: base_url + 'GeneralLeave/GetListDetail?Id=' + $("#id").val(), postData: { filters: null } }).trigger("reloadGrid");
    }
    ...
    //GRID +++++++++++++++
    $("#list").jqGrid({
        url: base_url + 'GeneralLeave/GetList',
        postData: { 'ParentId': function () { return $("#parenttype").val(); } },
        datatype: "json",
        colNames: ['ID', 'Start Date', 'End Date', 'Remark', 'Created At', 'Updated At'],
        colModel: [
    ...
    $('#btn_edit').click(function () {
        ...
        if (id) {
            ...
            $.ajax({
                dataType: "json",
                url: base_url + "GeneralLeave/GetInfo?Id=" + id,
    ...
    $('#delete_confirm_btn_submit').click(function () {
        $.ajax({
            url: base_url + "GeneralLeave/Delete",
            type: "POST",
    ...
    $("#form_btn_save").click(function () {
        ...
        if (id != undefined && id != '' && !isNaN(id) && id > 0) {
            submitURL = base_url + 'GeneralLeave/Update';
        }
            // Insert
        else {
            submitURL = base_url + 'GeneralLeave/Insert';
        }
    ...
    $("#listdetail").jqGrid({
        url: base_url,
        datatype: "json",
        colNames: ['GeneralLeave ID', 'SalaryItem ID', 'SalaryItem Code', 'SalaryItem Name', 'Amount'],
        colModel: [
    ...
    $('#btn_edit_detail').click(function () {
        ...
        if (id) {
            $.ajax({
                dataType: "json",
                url: base_url + "GeneralLeave/GetInfoDetail?Id=" + id,
                success: function (result) {
                    if (result.Id == null) {
                        $.messager.alert('Information', 'Data Not Found...!!', 'info');
                    }
    ...
    $('#btn_del_detail').click(function () {
        ...
        if (id) {
            ...
            $.messager.confirm('Confirm', 'Are you sure you want to delete record?', function (r) {
                if (r) {
                    $.ajax({
                        url: base_url + "GeneralLeave/DeleteDetail",
                        type: "POST",
                        contentType: "application/json",
                        data: JSON.stringify({
                            Id: id,
    ...
    $("#detail_btn_submit").click(function () {
        ...
        // Update
        if (id != undefined && id != '' && !isNaN(id) && id > 0) {
            submitURL = base_url + 'GeneralLeave/UpdateDetail';
        }
            // Insert
        else {
            submitURL = base_url + 'GeneralLeave/InsertDetail';
        }
        ...
        $.ajax({
            ...
            url: submitURL,
            data: JSON.stringify({
                Id: id, GeneralLeaveId: $("#id").val(), SalaryItemId: $("#SalaryItemId").val(), Amount: $("#Amount").numberbox('getValue'),
            }),
            async: false,
            cache: false,
            timeout: 30000,
