$(document).ready(function () {
    ...
    function UpdateEnable(objId, isEnabled) {
        ...
        $.ajax({
            ...
            type: 'POST',
            url: base_url + "EmployeeFamilyInfo/UpdateEnable?isEnabled=" + isEnabled,
            data: JSON.stringify({
                Id: objId
            }),
    ...
    function ReloadGridViewChildren(id) {
        ...
        $("#list_detail").jqGrid({
            url: base_url + 'EmployeeFamilyInfo/GetList?Id=' + id,
            //postData: { 'ParentId': function () { return $("#parenttype").val(); } },
            datatype: "json",
            colNames: ['ID', 'EmployeePersonal ID', 'EmployeePersonal NIK', 'EmployeePersonal Name', 'Name', 'Relationship', 'Birth Date', 'Job', 'Sex', 'Marital Status', 'Last Education', 'Is Deceased', 'Remark', 'Is Relative', 'Is Deleted', 'Created At', 'Updated At'],
            colModel: [
                      { name: 'id', index: 'id', width: 80, align: "center", frozen:true },
                      { name: 'employeepersonalid', index: 'employeepersonalid', width: 100, frozen: true, hidden: true },
                      { name: 'employeepersonalnik', index: 'employeepersonalnik', width: 100, frozen: true, hidden: true },
                      { name: 'employeepersonalname', index: 'employeepersonalname', width: 100, frozen: true, hidden: true },
                      { name: 'name', index: 'name', width: 100, frozen: true, },
                      { name: 'relationship', index: 'relationship', width: 100, stype: 'select', editoptions: { value: getSelectOption("#Relationship") } },
                      ...
                      { name: 'sex', index: 'sex', width: 100, stype: 'select', editoptions: { value: getSelectOption("#Sex") } },
                      { name: 'maritalstatus', index: 'maritalstatus', width: 100, stype: 'select', editoptions: { value: getSelectOption("#MaritalStatus") } },
                      { name: 'lasteducation', index: 'lasteducation', width: 100 },
                      { name: 'isdeceased', index: 'isdeceased', width: 100 },
                      { name: 'remark', index: 'remark', width: 100 },
                      { name: 'isrelative', index: 'isrelative', width: 100, hidden:true },
            ...
            gridComplete:
              function () {
                  ...
                  for (var i = 0; i < ids.length; i++) {
                      ...
                      rowRelation = document.getElementById("Relationship").options[$(this).getRowData(cl).relationship].text;
                      $(this).jqGrid('setRowData', ids[i], { relationship: rowRelation });
                  }
              }
    ...
    $('#btn_edit').click(function () {
        ...
        if (id) {
            ...
            $.ajax({
                dataType: "json",
                url: base_url + "EmployeeFamilyInfo/GetInfo?Id=" + id,
    ...
    $('#delete_confirm_btn_submit').click(function () {
        ...
        $.ajax({
            url: base_url + "EmployeeFamilyInfo/Delete",
            type: "POST",
            contentType: "application/json",
            data: JSON.stringify({
    ...
    $("#form_btn_save").click(function () {
        ...
        if (id != undefined && id != '' && !isNaN(id) && id > 0) {
            submitURL = base_url + 'EmployeeFamilyInfo/Update';
        }
            // Insert
        else {
            submitURL = base_url + 'EmployeeFamilyInfo/Insert';
        }
        ...
        $.ajax({
            ...
            url: submitURL,
            data: JSON.stringify({
                Id: id, EmployeeId: $("#EmployeeId").val(), EmployeePersonalId: jQuery("#list").jqGrid('getGridParam', 'selrow'), //IsEnabled: document.getElementById("IsEnabled").checked ? 'true' : 'false',
